...
function numberFormat (num) {
    return num.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ').replace(".", ",");
}
        ...
        if (els) {
            for (let i = 0, n = els.length; i < n; ++i) {
                if (els[i].nodeType == 3) {
                    ...
                    if (!(tag == "textarea" || tag == "style" || tag == "script" || tag == "noscript" || tag == "code" || tag == "head")) {
                        inspectText(els[i]);
                    }
                }
            }
...
function processPrice (str) {
    return str.replace(regmoney,
        function (str, p1, p2, offset, s) {
            //console.log(arguments);
            return USD2UAH(p1) + (p2 != undefined ? " - "+ USD2UAH(p2) : "") + " грн ($"+p1+ (p2 != undefined ? " - "+ p2 : "") + ")";
        });
}
